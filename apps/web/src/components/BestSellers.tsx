...
export function BestSellers() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError(null);
      try {
        const res = await fetch(`${API_BASE}/products?bestseller=true&limit=8`, { cache: "no-store" });
  ...
  return (
    <section className="py-16">
      <Container>
        <SectionHeading title="Best Sellers" subtitle="Customer favorites flying off the shelves" />
        {loading && (
